...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop Phoenixcoin daemon.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Phoenixcoin daemon stopped";
}
...
// Phoenixcoin: Return average network hashes per second based on last number of blocks.
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Phoenixcoin address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid Phoenixcoin address:")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Phoenixcoin server stopping, restart to run with encrypted wallet";
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
        result.push_back(Pair("algorithm", "scrypt:1024,1,1"));  // Phoenixcoin: specify that we should use the scrypt algorithm
        return result;
    }
    else
    {
...
//
// JSON-RPC protocol.  Phoenixcoin speaks version 1.0 for maximum compatibility,
